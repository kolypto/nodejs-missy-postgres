...
ModelUpdateQuery.prototype.customQuery = function(update, where, multi){
    ...
    if (multi)
        ...
        // Limit the number of output rows: SELECT sub-query
        var pk_fields = _.map(this.model.options.pk, function(c){ return escapeIdentifier(c); });
        ...
        ret.where = [
            ...
            '(',
                ...
                'FROM', this.table,
                'WHERE', prepareCriteria(this.tableName, where, ret.params),
                'LIMIT', addParam(ret.params, 1),
                ')'
        ].join(' ');
    }
